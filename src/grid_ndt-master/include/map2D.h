...
//                              cout<<"sloep.z-mean "<<(sit->second)->mean(2)-mean(2)<<endl;
    ...
    bool countNeighbor(string leftMtn, list<Slope *> & listm,Vector3 mean, float h){
        ...
        if(mit != map_cell.end()){
            ...
            while(sit != (mit->second)->map_slope.end()){
                if(sit->second->up != true){
                    float z_diff = abs(mean(2)- (sit->second->mean(2)));
...
//            cout<<"up collide\n";
        ...
        while(itSlope != allSlope.end()){
            // if(((*itSlope)->mean(2) > slope->mean(2)+2*r) && (*itSlope)->down == true){//yu update
            //     return true;//collide
            // }
            // if(((*itSlope)->mean(2) < slope->mean(2)) && (*itSlope)->countUp(map_xy,getInterval()) == true){//yu update
            //     return true;//collide
            // }
...
//            cout<<"up collide\n";
        ...
        while(itSlope != allSlope.end()){
            ...
            if(((*itSlope)->mean(2) < slope->mean(2)) && slope->mean(2) - (*itSlope)->mean(2) > robot.getReachableHeight()){
                return true;//collide
            }
            itSlope++;
        }
...
//    multimap<string,OcNode *,CmpByKeyUD> map_z;
    ...
    void getReachable(string leftMtn, list<Slope *> & listm,RobotSphere & robot,int morton_z,
                      Vector3f normal,Vector3 mean,float comand){
        countReachable(leftMtn, listm,robot,morton_z,normal,mean,comand);   
    }
    ...
    ///no use
  Slope *  findSlope(string & morton_xy,int & morton_z){
...
//                             for test
                             if(demand.compare("slope") == 0){
                                 if((it->second)->isSlope(map_xy,up,down,slope_interval) ){
                                     ...
                                     slope->up = up,slope->down = down;//yu update
...
//        cout<<"delete slope num "<<delete_num<<endl;
    ...
    //for visualization
    void countPositionXYZ(float & x,float &y,float &z,const string& s_xy,const int& s_z){
...
//         int mortonz = strToInt( s_z.substr(1,s_z.length()-1));
    ...
    float countPositionZ(const int& s_z){
        float z;
         if(s_z >0){
             z = cloudFirst(2) + (abs(s_z)-0.5)*zLen;
         }
    ...
    void transXYMorton(int & x,int &y,const string& s_xy){
         string belongxy = s_xy.substr(0,1);
         int mortonxy = strToInt( s_xy.substr(1,s_xy.length()-1));
         int a,b;
         mortonToXY(a,b,mortonxy);
    ...
    void transMortonXY(const int & x,const int &y,string& s_xy){
        string belongxy;
        if(x > 0 && y > 0)  belongxy = "A";
        if(x > 0 && y < 0)  belongxy = "B";
        if(x < 0 && y > 0)  belongxy = "C";
        if(x < 0 && y < 0)  belongxy = "D";
        string morton = countMorton(abs(x), abs(y));
        s_xy = belongxy + morton;
    }

        //tranform position into morton_xy and morton_z
    void transMortonXY(const Vector3& position, string & morton_xy){
        string xy_belong;
...
//        cout<<"abcd "<<t2-t1<<",nxyz "<<t3-t2<<",morton "<<t4-t3<<",str&float"<<t5-t4<<endl;
    ...
    void transMortonZ(const float& z, int & morton_z){
        if(z > cloudFirst(2)) morton_z = 1; //up
        else morton_z = -1; //down
        int nz = (int)ceil(float(abs(z-cloudFirst(2))/zLen));
        nz == 0? nz =1:nz=nz;
...
//                    m_s.pose.position.z = z;
    ...
    void showStartandGoal(ros::Publisher marker_pub,RobotSphere & robot){
...
//                         marker.color.r = 1;
    ...
    void computeCost(Vector3& goal,RobotSphere & robot,ros::Publisher marker_pub,ros::Publisher marker_pub2,string demand){
        ...
        float goal_z;
        transMortonXY(goal,morton_xy);
        if(!getZneast(morton_xy, goal(2), goal_z)){
            cout<<"cant find goal nearst z slope"<<endl;
        }
        goal(2) = goal_z;
        transMortonZ(goal_z, morton_z);
        robot.setGoal(Pose3d(goal(0), goal(1), goal(2)));
